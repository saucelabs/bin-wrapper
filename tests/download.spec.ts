...
describe('downloads', () => {
  ...
  test('Complete setup - tar', async () => {
    ...
    mockedFsPromises.writeFile.mockImplementation(
      (file: string | any, content: ArrayBuffer | any) => {
        return new Promise((resolve) => {
          usedFilename = file;
          usedBody = content;
          resolve();
    ...
    await downloadAndUnpack(
      new URL('http://dummy-host/archive.tar'),
      'dummy.txt',
      'dummy.txt',
    );
  ...
  test('File not available', async () => {
    ...
    try {
      ...
      expect(e).toEqual(
        new Error(
          'unable to find invalid-file in http://dummy-host/archive.tar',
        ),
      );
    }
  });
  ...
  test('Buggy download', async () => {
    ...
    try {
      ...
      expect(e).toEqual(
        new Error('failed to download: network-connection-broken'),
      );
    }
  });
});
...
test('Headers are carried over', async () => {
  mockedAxios.get.mockImplementation(
    (url: string, options: any): Promise<unknown> => {
